...
#if ODIN_INSPECTOR
        [ShowInInspector, PropertySpace(10, SpaceAfter = 20), TabGroup("基本", TextColor = "#82CD86")]
#else
        [SerializeField]
#endif
        [Tooltip("在输出中，把 prompt与suffix 的内容也输出出来")]
        private bool _echo;
...
#if ODIN_INSPECTOR
        [ShowInInspector, PropertySpace(10, SpaceAfter = 20), TabGroup("调节", TextColor = "#61AFEF"), LabelText("重复惩罚度-1")]
#else
        [SerializeField]
#endif
        [Range(-2.0F, 2.0F), Tooltip("介于 -2.0 和 2.0 之间的数字。如果该值为正，那么新 token 会根据其在已有文本中的出现频率受到相应的惩罚，降低模型重复相同内容的可能性。")]
...
#if ODIN_INSPECTOR
        [ShowInInspector, PropertySpace(10, SpaceAfter = 20), TabGroup("调节")]
#else
        [SerializeField]
#endif
        [Range(0, 20), Tooltip("制定输出中包含 logprobs 最可能输出 token 的对数概率，包含采样的 token。例如，如果 logprobs 是 20，API 将返回一个包含 20 个最可能的 token 的列表。" +
...
#if ODIN_INSPECTOR
        [ShowInInspector, PropertySpace(10, SpaceAfter = 20), TabGroup("调节"), LabelText("最大 token 数量")]
#else
        [SerializeField]
#endif
        [Range(1, 8192), Tooltip("最大输出 token 数量（不建议取大，尤其是代码生成场景）。")]
...
#if ODIN_INSPECTOR
        [ShowInInspector, PropertySpace(10, SpaceAfter = 20), TabGroup("调节"), LabelText("重复惩罚度-2")]
#else
        [SerializeField]
#endif
        [Range(-2, 2), Tooltip("介于 -2.0 和 2.0 之间的数字。如果该值为正，那么新 token 会根据其是否已在已有文本中出现受到相应的惩罚，从而增加模型谈论新主题的可能性。")]
...
#if ODIN_INSPECTOR
        [ShowInInspector, PropertySpace(10, SpaceAfter = 20), LabelText("停止词"), TabGroup("格式", TextColor = "#C678DD")]
#else
        [SerializeField]
#endif
        [Tooltip("一个 string 或最多包含 16 个 string 的 list，在遇到这些词时，API 将停止生成更多的 token。")]
        private string[] _stop = Array.Empty<string>();
...
#if ODIN_INSPECTOR
        [ShowInInspector, PropertySpace(10, SpaceAfter = 20), TabGroup("流式控制", TextColor = "#56B6C2")]
#else
        [SerializeField]
#endif
...
#if ODIN_INSPECTOR
        [ShowInInspector, LabelText("温度"), PropertySpace(10, SpaceAfter = 20), TabGroup("调节")]
#else
        [SerializeField]
#endif
